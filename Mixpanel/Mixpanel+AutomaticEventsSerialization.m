...
+ (NSDictionary *)propertiesForSource:(id)source {
    ...
    if ([source respondsToSelector:@selector(class)]) {
        ...
        if (text) {
            if ([source respondsToSelector:@selector(isSecureTextEntry)]) {
                if (![source isSecureTextEntry]) {
...
+ (NSDictionary *)propertiesForDestination:(id)destination {
    ...
    if ([destination respondsToSelector:@selector(class)]) {
        ...
        if (text) {
            if ([destination respondsToSelector:@selector(isSecureTextEntry)]) {
                if (![destination isSecureTextEntry]) {
...
+ (NSString *)textForElement:(id)element {
    if ([element respondsToSelector:@selector(titleForState:)]) {
